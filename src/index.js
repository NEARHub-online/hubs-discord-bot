...
function formatStats(stats, where, when) {
    const header = when != null ? `Nearhub activity in <${where}> for ${when}:\n` : `Nearhub activity in <${where}>:\n`;
...
async function updateChannelPresenceIcons(channels, active) {
    for (const channel of channels) {
        ...
        await channel.setName(updatedName, `Nearhub room became ${active ? "active" : "inactive"}.`);
        // }
    }
}
...
async function tryGetOrCreateWebhook(discordCh) {
    try {
        ...
        if (existingHook != null) {
            ...
            const newHook = await discordCh.createWebhook(process.env.HUBS_HOOK, DUCK_AVATAR, "Bridging chat between Nearhub and Discord.");
            discordCh.send(`Created a new webhook (${newHook.id}) to use for Nearhub chat bridging.`);
            return newHook;
        }
    } catch (e) {
...
async function establishBridging(hubState, bridges) {
    ...
    presenceRollups.on('new', ({ kind, users, fresh }) => {
        ...
        for (const discordCh of bridges.getChannels(hubState.id).values()) {
            ...
            if (kind === "arrive") {
                const msg = fresh ?
                    `${formatList(users)} joined the Nearhub room. Join them: ${hubState.shortUrl}` :
                    `${formatList(users)} joined the Nearhub room.`;
                lastPresenceMessages[discordCh.id] = discordCh.send(msg);
            } else if (kind === "depart") {
                const msg = `${formatList(users)} left the Nearhub room.`;
                lastPresenceMessages[discordCh.id] = discordCh.send(msg);
            } else if (kind === "rename") {
    ...
    reticulumCh.on('renamehub', (timestamp, id, whom, name, slug) => {
        for (const discordCh of bridges.getChannels(hubState.id).values()) {
            ...
            discordCh.send(`${whom} renamed the Nearhub room at ${hubState.shortUrl} to ${hubState.name}.`);
        }
    });
...
async function connectToHub(reticulumClient, discordChannels, host, hubId) {
    const reticulumCh = reticulumClient.channelForHub(hubId, serializeProfile("Nearhub Bot", discordChannels));
    reticulumCh.on("connect", (timestamp, id) => { console.info(ts(`Connected to Nearhub room ${hubId} with session ID ${id}.`)); });
...
async function start() {
    ...
    console.info(ts(`Scanning channel topics for Nearhub hosts: ${HOSTNAMES.join(", ")}`)); {
        ...
        for (const [key, channels] of candidateBridges.entries()) {
            ...
            try {
                ...
                for (const discordCh of channels) {
                    ...
                    console.info(ts(`Nearhub room ${hubState.id} bridged to ${formatDiscordCh(discordCh)}.`));
                }
                await establishBridging(hubState, bridges);
            } catch (e) {
                console.error(ts(`Error bridging Nearhub room ${hubId}:`), e);
            }
        }
    ...
    notificationManager.on('notify', async(_when, msg) => {
        ...
        const description = hubState != null ? `the Nearhub room: ${hubState.shortUrl}` : "a Nearhub room.";
            ...
            if (hubState != null) {
                try {
                    ...
                    if (oldWebhook != null && newWebhook == null) {
                        await discordCh.send("Webhook disabled; Nearhub will no longer bridge chat. Re-add a channel webhook to re-enable chat bridging.");
                    } else if (newWebhook != null && (oldWebhook == null || newWebhook.id !== oldWebhook.id)) {
                        await discordCh.send(`The webhook "${newWebhook.name}" (${newWebhook.id}) will now be used for bridging chat in Nearhub.`);
                    }
            ...
            try {
                if (prevHub != null && prevHub.id != currHubId) {
                    ...
                    if (bridgedChannels.size === 0) {
                        console.info(ts(`Disconnecting from Nearhub room ${prevHub.id}.`));
                        connectedHubs[prevHub.id] = null;
                        await prevHub.reticulumCh.close();
                    } else {
                        prevHub.reticulumCh.updateProfile(serializeProfile("Nearhub Bot", Array.from(bridgedChannels.values())));
                    }
                ...
                if (currHubId != null && (prevHub == null || prevHub.id != currHubId)) {
                    ...
                    if (currHub == null) {
                        ...
                        currHub.reticulumCh.updateProfile(serializeProfile("Nearhub Bot", Array.from(bridgedChannels.values())));
                    }
                    ACTIVE_WEBHOOKS[newChannel.id] = await tryGetOrCreateWebhook(newChannel);
                    console.info(ts(`Nearhub room ${currHubId} bridged to ${formatDiscordCh(newChannel)}.`));
    ...
    const HELP_PREFIX = "Hi! I'm the Nearhub bot. I connect Discord channels with rooms on Nearhub (<https://nearverse.club/>). Type `!nearhub-dev help` for more information.";

    const COMMAND_HELP_TEXT =
        "Command reference:\n\n" +
        "ðŸ¦† `!nearhub-dev` - Shows general information about the Nearhub integration with the current Discord channel.\n" +
        "ðŸ¦† `!nearhub-dev help` - Shows this text you're reading right now.\n" +
        "ðŸ¦† `!nearhub-dev create` - Creates a default Nearhub room and puts its URL into the channel topic. " +
        "Rooms created with `!nearhub-dev create` will inherit moderation permissions from this Discord channel and only allow Discord users in this channel to join the room.\n" +
        "ðŸ¦† `!nearhub-dev create [environment URL] [name]` - Creates a new room with the given environment and name, and puts its URL into the channel topic. " +
        "Valid environment URLs include GLTFs, GLBs, and Spoke scene pages.\n" +
        "ðŸ¦† `!nearhub-dev stats` - Shows some summary statistics about room usage.\n" +
        "ðŸ¦† `!nearhub-dev remove` - Removes the room URL from the topic and stops bridging this Discord channel with Hubs.\n" +
        "ðŸ¦† `!nearhub-dev notify set [datetime]` - Sets a one-time notification to notify @here to join the room at some future time.\n" +
        "ðŸ¦† `!nearhub-dev notify clear` - Removes all pending notifications.\n" +
        "ðŸ¦† `!nearhub-dev users` - Lists the users currently in the Nearhub room bridged to this channel.\n\n" +
    ...
    discordClient.on('message', msg => {
                ...
                if (args[0] !== "!nearhub-dev" && bridges.getHub(discordCh.id) == null) {
                    return;
                }
                ...
                q.enqueue(async() => {
                            ...
                            if (!discordCh.guild) { // e.g. you DMed the bot
                                return discordCh.send(HELP_PREFIX + "\n\n" +
                                    ...
                                    "If you're curious about what I do, try `!nearhub-dev help` or check out https://github.com/MozillaReality/hubs-discord-bot."
                                );
                            }

                            // echo normal chat messages into the hub, if we're bridged to a hub
                            const hubState = bridges.getHub(discordCh.id);
                            if (args[0] !== "!nearhub-dev") {
                                if (hubState == null) {
                                    return;
                                }
                            ...
                            switch (args[1]) {
                                ...
                                case undefined:
                                    {
                                        // "!nearhub-dev" == emit useful info about the current bot and hub state
                                        if (hubState != null) {
                                            const userCount = Object.values(hubState.reticulumCh.getUsers()).length;
                                            return discordCh.send(
                                                    HELP_PREFIX + `.\n\n` +
                                                    `ðŸ¦† <#${discordCh.id}> bridged to Nearhub room "${hubState.name}" (${hubState.id}) at <${hubState.url}>.\n` +
                                                    `ðŸ¦† ${activeWebhook ? `Bridging chat using the webhook "${activeWebhook.name}" (${activeWebhook.id}).` : "No webhook configured. Add a channel webhook to bridge chat to Nearhub."}\n` +
          ...
          return discordCh.send(
            HELP_PREFIX + `.\n\n` +
              `ðŸ¦† This channel isn't bridged to any room on Nearhub. Use \`!nearhub-dev create\` to create a room, or add an existing Nearhub room to the topic to bridge it.\n`
          );
        }
      }

      case "help": {
        // !nearhub-dev help == bot command reference
      ...
      case "create": {
        ...
        if (topicManager.matchHub(discordCh.topic)) {
          return discordCh.send("A Nearhub room is already bridged in the topic, so I am cowardly refusing to replace it.");
        }
        ...
        if (sceneId) { // !nearhub-dev create [scene URL] [name]
        ...
        } else { // !nearhub-dev create [environment URL] [name]
      ...
      case "remove": {
        // "!nearhub-dev remove" == if a hub is bridged, remove it
      ...
      case "users": {
        ...
        if (hubState != null) {
          ...
          return discordCh.send("No Nearhub room is currently bridged to this channel.");
        }
      }

      case "stats": {
        // "!nearhub-dev stats" == stats for the current hub
        if (hubState != null) {
      ...
      case "notify": {
        ...
        if (args.length === 3) { // !nearhub-dev notify clear
          const pins = await discordCh.fetchPinnedMessages();
          const notifications = pins.filter(msg => {
            return msg.author.id === discordClient.user.id && NotificationManager.parseTimestamp(msg).isValid();
          });
        ...
        } else { // !nearhub-dev notify set [date/time descriptor]
